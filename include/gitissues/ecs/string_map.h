...
// TODO: if we want to provide the guarantee that component IDs are produced
...
struct StringMapNode {
  ...
  // notably we never expect to use the full component id, so we can encode an
  // extra byte there too? compare just the high bits of the hash (low bits will
  // determine index generally, and thus are of lower importance)
  uint32_t stringHash;
...
_Static_assert(sizeof(struct StringMapNode) == 24,
               "StringMapNode had unexpected padding");

// TODO: should consider
// - we could store value types inline, with a vector storing the actual
// metadata?
// - at minimum, want a vector of just the component ids, easily iterable?
// - string map nodes instead just point to the component id? (extra
// indirection?)

struct StringMap {
  // TODO: consider reduction to an index, at cost to another access
  struct StringMapNode *data;
  uint32_t size;
  uint32_t capacity;
  uint32_t threshold;

  // TODO: size field is shared
  // TODO: growth factor of capacity is shared, but shouldn't be
  struct {
    struct StringMapNode *data;
    uint32_t size;
    uint32_t capacity;
